...
function home(){
  main.empty();
  console.log('hi')
  $('#header').append('<img id="header-image" class="img-responsive" src="http://www.villalevistyle.com/wp-content/uploads/2014/09/Contact.jpg">')
}


//creating the chart on the charts page
function analytics(){
  main.empty();
  $.ajax({
    url: "/contacts",
  ...
  //checking each person city
  data.forEach(function(e){
    var city = e.city

    //if city does exist push into labels array for chart
    //and 1 value into the data amount array
    if(labelsArray.indexOf(city) === -1){
      labelsArray.push(city);
      dataAmount.push(1)

    // if city already exists it finds the index value of the city
    //then adds a value of one to that index in the dataAmount array
    } else {
...
main.on('click', '[data-action="submit-new-contact"]', function(){
  ...
  var image_url = info.find('[data-input="url"]').val();
  ...
  var category_id = "snaptarget All"
  ...
  $.ajax({
    ...
    console.log(data)
    var newContact = Mustache.render(singleContactTemplate, {id: data.id, name: name, email: email, image_url: image_url, category_id: category_id, phone: phone, city: city});
